...
#define GetByte(X) ((X[0] >= '0' && X[0] <= '9' ? (X[0] - '0') : ((X[0] & (~ 0x20)) - 'A' + 0xA)) << 4 | \
    ...
    {
        Create_Detour(Address, Disassemble(Address, 0x5), Destination, Detour);
    }

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    void Create_Detour(uintptr_t Address, uint32_t Requirement, uintptr_t Destination, Detour & Detour)
    {
        Detour.Destination = (uintptr_t) ::VirtualAlloc(
            nullptr, Requirement + 0x05, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        ::RtlCopyMemory((void *) Detour.Destination, (void *) (Detour.Source = Address), Requirement);
        ...
        Write_Opcode_Jump(Detour.Destination + Requirement, Address + Requirement);
    }
